    ...
    return (
        <>
            {order.selectedOptions.length > 0 && (
                <>
                    <h4 className="text-m text-text-secondary mb-1 text-left kanit-light mt-3">Additional options:</h4>
                    <ul className="space-y-2">
                        {order.selectedOptions.map((option, idx) => (
                            <li key={idx} className="flex items-start">
                                        <span
                                            className="inline-block w-2 h-2 mt-2 mr-2 bg-primary rounded-full"></span>
                                <div className="flex">
                                    <div
